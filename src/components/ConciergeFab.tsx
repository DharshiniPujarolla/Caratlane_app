...
export function ConciergeFab() {
  ...
  return (
    <div ref={ref} className="fixed bottom-28 right-4 z-[70] flex flex-col items-end gap-2">
      {options.map(({ label, icon: Icon }, i) => (
        <button
          ...
          className={cn(
            "press flex min-h-11 items-center gap-2.5 rounded-2xl border border-border bg-card py-2.5 pl-3 pr-4 shadow-card transition-all duration-300 hover:bg-primary-soft hover:shadow-float",
            open
              ? "pointer-events-auto translate-y-0 scale-100 opacity-100"
              : "pointer-events-none translate-y-3 scale-95 opacity-0",
          )}
        >
          <span className="flex h-11 w-11 min-h-11 min-w-11 items-center justify-center rounded-xl bg-primary-soft text-primary">
            <Icon size={16} />
          </span>
      ...
      <button
        ...
        className="press flex h-14 w-14 min-h-14 min-w-14 items-center justify-center rounded-full gradient-primary text-primary-foreground shadow-float transition-transform duration-300"
      >
